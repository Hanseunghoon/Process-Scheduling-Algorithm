...
#define SORT_remain_t   0x00000002
...
typedef struct process {
	...
	int remain_t; //수행후 남아있는 시간
	int wait_t; //대기시간
	int end_t; //종료시간
	int judge; //실행 판단(0or1)
} process;
...
void SPN(int cnt)
{
	...
	for (i = 0; !is_all_finish(); i++)
	{
		...
		//수행 후 남아있는 시간 줄임
		prc_queue[q_head]->remain_t--;

		//남아있는 수행시간이 없다면 프로세스 출력
		/* \nPID : %d, 도착시간 : %ds , 서비스시간 : %ds 종료시간 : %ds, 반환시간 : %ds, 정규화된 반환시간 : %.2fs\n\n */
		if (prc_queue[q_head]->remain_t == 0) {
			prc_queue[q_head]->is_finished = 1;
			printf("%d \t\t %ds\t\t  %ds\t\t  %ds\t\t %ds\t\t %.2fs \n\n", prc_queue[q_head]->pid, prc_queue[q_head]->ar_t, prc_queue[q_head]->ser_t, i + 1, (i + 1) - prc_queue[q_head]->ar_t, (double)(prc_queue[q_head]->wait_t + prc_queue[q_head]->ser_t) / (double)prc_queue[q_head]->ser_t);
	...
	for (i = 0; i <= pr_cnt; i++)
	{
		average_wait += g_process[i].wait_t;
	}
	printf("\n평균 대기 시간: %.2f\n", (float)average_wait / i);

	for (i = 0; i <= pr_cnt; i++)
	{
		/*printf("P%d : 대기:%d, 서비스:%d, 대기+서비스/서비스: %.2f \n", g_process[i].pid, g_process[i].wait_t, g_process[i].ser_t, (double)(g_process[i].wait_t + g_process[i].ser_t) / (double)g_process[i].ser_t);*/
		average_banhwan += (double)(g_process[i].wait_t + g_process[i].ser_t) / (double)g_process[i].ser_t;
		//printf("반환시간더하기: %.2f\n", average_banhwan);
	}
...
void RR(int cnt, process s[]) {
	...
	/* time quntum=4 */
	int quntum = 4, time = 0;
	int longWait = 0;//가장 오래기다린 프로세스
	int inter = 1;//colck 인터럽트 발생 여부 판단 변수
	...
	while (1) {
		for (int i = 0; i < cnt; i++) {
			...
			if (g_process[i].ar_t == time) {
				g_process[i].judge = 1;
			}
			//프로세스 생성 && 클럭인터럽트 발생 
			if (g_process[i].judge == 1 && inter == 1) {
				//ser_t != 0 && 가장 오래 기다린 프로세스의 대기시간보다 큰 대기시간을 가질때
				if (g_process[i].ser_t != 0 && g_process[i].wait_t >= g_process[longWait].wait_t) {
					//longWait의 인덱스 값 저장
					longWait = i;
				}
			}
		...
		//실행 후 대기시간 0으로 초기화
		g_process[longWait].wait_t = 0;

		//실행 했으므로 수행 시간 -1
		g_process[longWait].ser_t--;
		...
		for (int i = 0; i < cnt; i++) {
			...
			if (i != longWait && g_process[i].judge == 1) {
				g_process[i].wait_t++; //대기시간 +1
			}
		}

		//가장 오래 기다린 프로세스의 수행시간이 0 && 4초 이상 실행 시 중단
		if (g_process[longWait].ser_t != 0 && tmp_t != quntum) {
			inter = 0;
		}

		else {
			//수행시간이 0일 때 프로세스 실행 종료
			if (g_process[longWait].ser_t == 0) {
				tm = longWait;
				g_process[tm].judge = 0;
...
int main()
{
	...
	while (fgets(c, MAX_PROCESS, file) != NULL) {
		...
		a = strtok(NULL, ",");
		g_process[count].remain_t = g_process[count].ser_t = atoi(a);
		g_process[count].wait_t = 0;
		count++;
